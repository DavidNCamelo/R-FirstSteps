...
También, de acuerdo a r4ds permiten automatizar tareas comunes de una forma más potente y general que copiar y pegar. Escribir una función tiene cuatro grandes ventajas sobre copiar y pegar:

- Puedes darle a una función un nombre evocador que haga que tu código sea más fácil de entender.

- A medida que cambian los requisitos, solo necesita actualizar el código en un solo lugar, en lugar de en muchos.

- Elimina la posibilidad de cometer errores incidentales al copiar y pegar (es decir, actualizar un nombre de variable en un lugar, pero no en otro).

- Facilita la reutilización del trabajo de un proyecto a otro, lo que aumenta su productividad a lo largo del tiempo.

Se suelen clasificar en 3 tipos:

1. Las funciones vectoriales toman uno o más vectores como entrada y devuelven un vector como salida.
2. Las funciones de marco de datos toman un marco de datos como entrada y devuelven un marco de datos como salida.
3. Funciones de gráfico que toman un marco de datos como entrada y devuelven un gráfico como salida.
...
Es decir si exiten vectores con contenido númerico y luego se le incluye un _string_ este se convertirá en _character_
...
Incluso, es posible convertir booleano en números a través de _as.numeric()_
...
ch <- ggplot(data = penguins, aes(x = flipper_length_mm, y = body_mass_g)) +
  coord_flip()
...
- Comentarios; Con una claridad de "_Usa comentarios para explicar el porqué de tu código, no el cómo ni el qué . El qué y el cómo de tu código siempre se pueden entender, aunque sea tedioso, leyéndolo con atención._"
...
A nivel de estilo, (que debería ser regla en varios lenguajes), mantener espacios durante la escritura de código "_Coloque espacios a ambos lados de los operadores matemáticos excepto ^(es decir  +, -, ==, <, …)_" y "_No deje espacios dentro ni fuera de los paréntesis en las llamadas a funciones regulares_"
...
E incluso, como elemento extra, se habla de proyectos, que consiste en crear un Workspace de trabajo desde la configuración de RStudio para gestionar multiples análsis en "scripts" separados desde una única ubicación de la carpeta.
