...
p <- ggplot(mtcars, aes(x = wt, y = mpg, color = factor(cyl))) +
  geom_point(size = 3) +
  labs(
    title = "Weight vs Consumption of Gasoline Relationship",
    x = "Weight (1000 lbs)",
    y = "Miles by gallon",
    color = "Cilinders"
  )
...
p <- ggplot(mtcars, aes(x = wt, y = mpg, color = factor(cyl))) +
  geom_point_interactive(
    aes(tooltip = paste("Weight:", wt, "<br>MPG:", mpg, "<br>Cilinders:", cyl)), # Personalized Tooltip
    size = 4
  ) +
  labs(
    title = "Weight vs Consumption of Gasoline Relationship",
...
p <- ggplot(df, aes(x = category, y = value)) +
  geom_bar_interactive(
    stat = "identity",
    aes(
      fill = category, # Highlighting the bars
...
p <- ggplot(df2, aes(x = date, y = sales)) +
  geom_line_interactive(
    aes(tooltip = paste("date:", date, "<br>sales:", sales)),
    color = "blue",
    size = 1
  ) +
  geom_point_interactive(
  ...
  labs(title = "Monthly sales behavior", x = "date", y = "sales")
